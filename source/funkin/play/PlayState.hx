...
class PlayState extends MusicBeatState
{
	...
	public static var dad:BaseCharacter;
	public static var gf:BaseCharacter;
	public static var boyfriend:BaseCharacter;
	...
	override public function create()
	{
		...
		var songLowercase = formatSongName(PlayState.SONG.song);
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			stage.onLightningStrike = function()
			{
				boyfriend.playAnim('scared', true);
				gf.playAnim('scared', true);
			};

			if (curStage == 'school' && songLowercase == 'roses' && FlxG.save.data.distractions)
				stage.event('girlsScared');
		}
		else
		{
			curStage = stageCheck;
		...
		gf = new BaseCharacter(400, 130, gfCheck);
		gf.scrollFactor.set(0.95, 0.95);

		dad = new BaseCharacter(100, 100, SONG.player2);
		...
		boyfriend = new BaseCharacter(770, 450, SONG.player1, true);

		if (stage != null)
		{
			var bfOffset = stage.getCharacterPosition('bf');
			if (bfOffset != null)
			{
				boyfriend.x += bfOffset.x;
				boyfriend.y += bfOffset.y;
			}

			var gfOffset = stage.getCharacterPosition('gf');
			if (gfOffset != null)
			{
				gf.x += gfOffset.x;
				gf.y += gfOffset.y;
			...
			var dadOffset = stage.getCharacterPosition('dad');
			if (dadOffset != null)
			{
				dad.x += dadOffset.x;
				dad.y += dadOffset.y;
			...
			if (curStage == 'limo' && FlxG.save.data.distractions)
				stage.resetFastCar();

			if (curStage == 'schoolEvil' && FlxG.save.data.distractions)
				stage.addTrail(dad);
		}
		...
		kadeEngineWatermark = new FlxText(4, healthBarBG.y + 50, 0,
			SONG.song + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | KE " + MainMenuState.kadeEngineVer : ""), 16);
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var lowerSong = formatSongName(PlayState.SONG.song);

		if (lowerSong == 'roses' || lowerSong == 'thorns')
		{
			remove(black);
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			#if windows
			DiscordClient.changePresence("PAUSED on " + SONG.song + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy),
				"Acc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses, iconRPC);
			#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			{
				DiscordClient.changePresence(detailsText + " " + SONG.song + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy),
					"\nAcc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses, iconRPC, true,
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if ((daNote.mustPress && daNote.tooLate && !PlayStateChangeables.useDownscroll || daNote.mustPress && daNote.tooLate && PlayStateChangeables.useDownscroll)
					&& daNote.mustPress)
				{
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					if (formatSongName(PlayState.storyPlaylist[0]) == 'eggnog')
					{
						var blackShit:FlxSprite = new FlxSprite(-FlxG.width * FlxG.camera.zoom, -FlxG.height * FlxG.camera.zoom).makeGraphic(
							FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
